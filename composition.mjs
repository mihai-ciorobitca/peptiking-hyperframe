...
export function buildComposition(plan, { mainHasAudio = true, hasMusic = false } = {}) {
  let start = 0
  const clips = plan.segments.map((segment, index) => {
    const duration = segment.sourceEnd - segment.sourceStart
    const clip = `<video id="main-${index}" class="clip footage" src="assets/main.mp4" data-start="${start}" data-duration="${duration}" data-media-start="${segment.sourceStart}" data-track-index="0" style="transform:scale(${segment.zoom})" ${mainHasAudio && !plan.muteOutput ? `data-has-audio="true" data-volume="${plan.speechVolume}"` : 'muted data-volume="0"'} playsinline></video>`
    start += duration
    return clip
  })
  ...
  const fades = plan.brolls.map((clip, index) => {
    ...
    return `tl.fromTo('#broll-${index}',{opacity:0},{opacity:1,duration:${fade}},${clip.start});tl.to('#broll-${index}',{opacity:0,duration:${fade}},${clip.start + clip.duration - fade});`
  }).join('\n')
...
<script>window.__timelines=window.__timelines||{};const tl=gsap.timeline({paused:true});tl.to({}, {duration:${plan.duration}},0);${fades}window.__timelines.main=tl;</script></body></html>`
}
