...
test('asset plan caps generation and never acquires duplicate or muted music', () => {
  ...
  assert.equal(validateAssetPlan({ ...value, scenes: Array(6).fill(scene) }, context).scenes.length, 6)
  assert.throws(() => validateAssetPlan({ ...value, scenes: Array(7).fill(scene) }, context))
  assert.throws(() => validateAssetPlan({ ...value, scenes: Array(6).fill(scene) }, { ...context, brollCount: 15 }))
