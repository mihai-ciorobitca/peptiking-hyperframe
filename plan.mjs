...
export const planSchema = object({
  ...
  summary: text,
  segments: { type: 'array', items: object({ sourceStart: number, sourceEnd: number, zoom: number, motion: { anyOf: [{ type: 'null' }, object({ zoomFrom: number, zoomTo: number, transitionSeconds: number, followFace: { type: 'boolean' }, faceTrackId: { type: ['integer', 'null'] }, targetX: number, targetY: number })] } }) },
  brolls: { type: 'array', items: object({ index: { type: 'integer', minimum: 1 }, start: number, duration: number, sourceStart: number }) },
...
export function validatePlan(plan, media) {
  ...
  for (const segment of plan.segments) {
    ...
    if (segment.motion != null) {
      ...
      if (!between(m.zoomFrom, 1, 1.5) || !between(m.zoomTo, 1, 1.5) || !between(m.transitionSeconds, 0.1, 5) || typeof m.followFace !== 'boolean' || !between(m.targetX, 0, 1) || !between(m.targetY, 0, 1) || !(m.faceTrackId === null || (Number.isInteger(m.faceTrackId) && m.faceTrackId > 0))) throw new Error('Invalid animated crop settings.')
    }
