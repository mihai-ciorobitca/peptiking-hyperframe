...
await run(ffmpeg, ['-y', '-f', 'lavfi', '-i', 'sine=frequency=880:sample_rate=44100', '-t', '1', path.join(projectDir, 'assets/music.mp3')], { signal })
const media = { main: { duration: 3, hasAudio: true }, brolls: [{ duration: 1 }], music: { duration: 1 } }
const plan = validatePlan({ unsupportedReason: null, summary: 'Local renderer verification', segments: [{ sourceStart: 0.5, sourceEnd: 2.5, zoom: 1 }], brolls: [{ index: 1, start: 0.5, duration: 0.8, sourceStart: 0 }], captions: [{ text: 'HyperFrames · PeptiKing', start: 0, end: 2 }], titles: [], captionColor: '#ffffff', accentColor: '#d7af58', musicVolume: 0.1, speechVolume: 1, muteOutput: false }, media)
...
for (const frequency of [440, 880]) {
  const levels = await run(ffmpeg, ['-i', output, '-vn', '-af', `bandpass=f=${frequency}:width_type=h:w=30,volumedetect`, '-f', 'null', '-'], { signal })
  const level = Number(levels.match(/mean_volume:\s*([-\d.]+) dB/)?.[1])
  if (!Number.isFinite(level) || level < -65) throw new Error(`The exported mix is missing the ${frequency === 440 ? 'source audio' : 'background music'} tone.`)
}
console.log(`PASS 1080×1920 MP4 with trim, captions, B-roll, source audio and looped/faded background music: ${output}`)
